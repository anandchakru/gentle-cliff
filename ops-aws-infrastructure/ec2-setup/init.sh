...
if [ -z "$JAR_NAME" ]; then
    ...
    echo "Example: $0 gentle-cliff-api-1.0.123.1-SNAPSHOT.jar"
    exit 1
fi
...
sudo mkdir -p /opt/gentle-cliff/ec2-setup
sudo mkdir -p /opt/gentle-cliff/lib
sudo mkdir -p /opt/gentle-cliff/env
sudo mkdir -p /opt/gentle-cliff/logs
sudo mkdir -p /opt/gentle-cliff/backups
...
# 4. Create gentle-cliff user if not exists
echo "=== Creating gentle-cliff user ==="
if ! id "gentle-cliff" &>/dev/null; then
    sudo useradd -r -s /bin/bash -d /opt/gentle-cliff -c "gentle-cliff API Service" gentle-cliff
    echo "User 'gentle-cliff' created successfully"
else
    echo "✅ User 'gentle-cliff' already exists"
fi
...
# Determine environment for S3 bucket (fallback to spring profile or prod)
...
echo "Source: s3://gentle-cliff-${ENV_FOR_S3}-ops/ec2-setup/"
echo "Target: /opt/gentle-cliff/ec2-setup/"

# Create target directory if it doesn't exist
sudo mkdir -p /opt/gentle-cliff/ec2-setup

# Sync all files from S3 ec2-setup directory
if sudo aws s3 sync s3://gentle-cliff-${ENV_FOR_S3}-ops/ec2-setup/ /opt/gentle-cliff/ec2-setup/; then
    echo "✅ Successfully synced EC2 setup scripts from S3"
    
    # Make all shell scripts executable
    sudo chmod +x /opt/gentle-cliff/ec2-setup/*.sh
    
    # Set ownership for all files
    sudo chown -R gentle-cliff:gentle-cliff /opt/gentle-cliff/ec2-setup/
    
    echo "📋 Downloaded EC2 setup files:"
    ls -la /opt/gentle-cliff/ec2-setup/
else
...
if [ -f "/opt/gentle-cliff/ec2-setup/gentle-cliff-api.service" ]; then
    sudo cp /opt/gentle-cliff/ec2-setup/gentle-cliff-api.service /etc/systemd/system/
    sudo systemctl daemon-reload
    sudo systemctl enable gentle-cliff-api.service
    echo "✅ Systemd service configured successfully"
else
    echo "❌ Service file not found: /opt/gentle-cliff/ec2-setup/gentle-cliff-api.service"
    echo "⚠️ Manual service setup may be required"
fi

# 7. Set ownership
echo "=== Setting ownership ==="
sudo chown -R gentle-cliff:gentle-cliff /opt/gentle-cliff
...
# Determine environment for secret name (fallback to spring profile or prod)
if [ -n "$SPRING_PROFILE" ]; then
    ENV_FOR_SECRET="$SPRING_PROFILE"
    echo "Using Spring profile for environment: $SPRING_PROFILE"
else
    ENV_FOR_SECRET="prod"
    echo "No Spring profile specified, defaulting to prod"
fi

# Single secret name for all application and database secrets
APP_SECRET_NAME="gentle-cliff-${ENV_FOR_SECRET}-app-secrets"
...
# Add Spring profile
if [ -n "$SPRING_PROFILE" ]; then
    echo "SPRING_PROFILES_ACTIVE=$SPRING_PROFILE" > "$TEMP_SPRING_ENV"
else
    echo "SPRING_PROFILES_ACTIVE=prod" > "$TEMP_SPRING_ENV"
fi
...
sudo mv "$TEMP_SPRING_ENV" /opt/gentle-cliff/env/spring.env
sudo chown gentle-cliff:gentle-cliff /opt/gentle-cliff/env/spring.env
sudo chmod 644 /opt/gentle-cliff/env/spring.env

echo "📋 Final Spring environment configuration:"
echo "SPRING_PROFILES_ACTIVE=$(grep SPRING_PROFILES_ACTIVE /opt/gentle-cliff/env/spring.env | cut -d'=' -f2)"
echo "Additional environment variables: $(grep -v SPRING_PROFILES_ACTIVE /opt/gentle-cliff/env/spring.env | wc -l)"
...
if [ -f "/opt/gentle-cliff/lib/app.jar" ]; then
    ...
    sudo cp /opt/gentle-cliff/lib/app.jar /opt/gentle-cliff/lib/$BACKUP_NAME
...
if sudo aws s3 cp "s3://gentle-cliff-${ENV_FOR_S3}-ops/jars/$JAR_NAME" /opt/gentle-cliff/lib/app.jar; then
    echo "✅ Successfully downloaded $JAR_NAME from S3"
    sudo chown gentle-cliff:gentle-cliff /opt/gentle-cliff/lib/app.jar
    sudo chmod 644 /opt/gentle-cliff/lib/app.jar
else
...
# 11. Restart service
echo "=== Restarting gentle-cliff-api service with profile: $SPRING_PROFILE ==="
sudo systemctl restart gentle-cliff-api.service
...
while [ $RETRY_COUNT -lt $MAX_RETRIES ]; do
    if systemctl is-active --quiet gentle-cliff-api.service; then
        ...
        echo
        echo "=== Recent Logs ==="
        sudo journalctl -u gentle-cliff-api.service -n 10 --no-pager
        echo
        echo "=== Current JAR ==="
