...
# gentle-cliff API Service Startup Script

# Set up logging
LOG_DIR="/opt/gentle-cliff/logs"
LOG_FILE="$LOG_DIR/gentle-cliff-$(date +%Y%m%d).log"
...
log_with_timestamp "Starting gentle-cliff API Service"
...
if [ ! -f "/opt/gentle-cliff/lib/app.jar" ]; then
    log_with_timestamp "ERROR: JAR file not found at /opt/gentle-cliff/lib/app.jar"
    exit 1
fi
...
log_with_timestamp "JAR file: $(ls -la /opt/gentle-cliff/lib/app.jar)"
...
if [ -n "$SPRING_PROFILES_ACTIVE" ]; then
    ...
    log_with_timestamp "No Spring profile specified, using prod (default)"
fi

# Start the application with logging
log_with_timestamp "Starting Java application: /opt/gentle-cliff/lib/app.jar $SPRING_ARGS"

# Run the application and pipe all output to our logging function
java -jar /opt/gentle-cliff/lib/app.jar $SPRING_ARGS 2>&1 | while IFS= read -r line; do
